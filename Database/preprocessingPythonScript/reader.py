...
class Reader():
    def __init__(self, account_gen_obj):
        self.first_names = ["Ming Sze", "Pui Ling", "Chun Yu", "Tin Lok", "Tak Shing", "Ling Kiu", "Wing Fu",
                            "Po Lam", "Kan Hei"]
        self.last_names = ["LEE", "CHAN", "HO", "CHEUNG", "WONG", "AU", "YEUNG", "DING", "CHENG", "LAM", "MO", "KUNG",
                           "TANG"]
        self.genders = ["M", "F"]
        self.email_suffixes = ["@gmail.com", "@hotmail.com", "@yahoo.com", "@my.cityu.edu.hk", "@rocketmail.com"]
        self.number_of_reader = 0
        if len(account_gen_obj.usernames) % 2 == 1:
            self.number_of_reader = int((len(account_gen_obj.usernames) + 1) / 2)
        else:
            self.number_of_reader = int(len(account_gen_obj.usernames) / 2)
